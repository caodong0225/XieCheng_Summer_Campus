...
import { getMyFavourite } from '../../api/user';
...
export default function MyLikesScreen() {
  ...
  const fetchSelected = async (type: 'note' | 'video' = selectedType) => {
    try {
      ...
      let response;
      if (type === 'note') {
        response = await getMyFavourite(params);
        ...
        if (response && response.success && response.list) {
          ...
          setError(response?.message || '获取点赞视频失败');
        }
      }
    } catch (err: any) {
      setError(`获取点赞${selectedType === 'note' ? '游记' : '视频'}时发生错误: ` + (err.message || '未知错误'));
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={tw`flex-1 justify-center items-center p-4 bg-white`}>
              ...
              <Text style={tw`text-gray-500 text-lg mb-2`}>还没有点赞视频</Text>
              <Text style={tw`text-gray-400 text-sm`}>去视频页面点赞喜欢的视频吧</Text>
            </View>
          }
          refreshControl={
            <RefreshControl
